...
def my_loss(output, giou, margin = 0.01):
    img_seg, word_seg = torch.split(output, 512, dim=1)
    
    #cos = nn.CosineSimilarity(dim=1, eps=1e-6)
    #cos_sim = cos(img_seg, word_seg)    
    cos_sim = torch.sum(img_seg * word_seg, dim=1)
    se = ((giou - cos_sim)/2).pow(2)
    margined_se = torch.max(torch.cuda.FloatTensor([0]), se - margin)
    margined_mse = margined_se.mean()
    return margined_mse
...
def get_torch_data(sample):
    ...
    GIoU =  sample['GIoU'].to(device).float()
    return image, bbox_image, loc_rel, embedding, GIoU
